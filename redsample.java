...
public class redsample {
    public static void main(String[] args) {
        ...
        myBot.runAction(myBot.getDrive().actionBuilder(new Pose2d(-9, 61, Math.PI + Math.PI/2))

                .strafeToConstantHeading(new Vector2d(-10, 32))
                .waitSeconds(1)
                .strafeToConstantHeading(new Vector2d(-27, 32))
                .splineToLinearHeading(new Pose2d(-48, 12, Math.PI *0),Math.PI)
                .waitSeconds(1)
                .strafeToConstantHeading(new Vector2d(-48, 55))
                .waitSeconds(1)
                .strafeToConstantHeading(new Vector2d(-48, 19))
                .splineToConstantHeading(new Vector2d(-57, 15), Math.PI) // vaipro 2 sample
                .strafeToConstantHeading(new Vector2d(-57, 55))//empurra
                .waitSeconds(1)
                .strafeToConstantHeading(new Vector2d(-57, 19))
                .splineToConstantHeading(new Vector2d(-63, 14), Math.PI)// vai pro 3 sample
                .strafeToConstantHeading(new Vector2d(-63, 55)) //empurra
                .waitSeconds(1)
                .strafeToConstantHeading(new Vector2d(-60, 20))
                .splineToConstantHeading(new Vector2d(-26, 12), Math.PI * 0)//estacionar
                .build());
